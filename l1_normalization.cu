...
__global__ void l1_normalization_warp_shf(const float* __restrict__ X, float* __restrict__ Y, size_t cols){
    extern __shared__ float s_sum[];
...
extern "C" void solution(const float* X, float* Y, size_t B, size_t D) {
    size_t threads = 1024;
    size_t warps_per_block = threads / 32;
    size_t blocks = B;

    l1_normalization_warp_shf<<<blocks,threads, warps_per_block * sizeof(float)>>>(X,Y,D);
}